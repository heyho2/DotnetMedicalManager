...
namespace GD.Consumer
{
    ...
    public class IndicatorWarningLimitBiz : BaseBiz<IndicatorWarningLimitModel>
    {
        ...
        /// <returns></returns>
        public async Task<IndicatorWarningLimitModel> GetModelAsyncByUserAndOption(string userId, string option, bool enable = true)
        {
            using (var conn = MySqlHelper.GetConnection())
            {
                return await conn.QueryFirstOrDefaultAsync<IndicatorWarningLimitModel>("SELECT *  FROM t_consumer_indicator_warning_limit where user_guid=@UserId and indicator_option_guid=@OptionId and turn_on_warning=1 and `enable`=@enable", new { UserId = userId, OptionId = option, enable });
            }
        }
