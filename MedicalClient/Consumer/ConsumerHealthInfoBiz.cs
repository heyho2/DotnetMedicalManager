...
namespace GD.Consumer
{
    ...
    /// 用户健康信息业务类
    /// </summary>
    public class ConsumerHealthInfoBiz : BizBase.BaseBiz<ConsumerHealthInfoModel>
    {
        /// <summary>
        /// 查找用户信息问题
        /// </summary>
        ...
        public async Task<ConsumerHealthInfoModel> GetConsumerHealthInfoAsync(string informationGuid, string userId)
        {
            ...
            {
                var result = await conn.GetListAsync<ConsumerHealthInfoModel>("where information_guid=@informationGuid and user_guid=@userId and `enable`=1", new { informationGuid, userId });
