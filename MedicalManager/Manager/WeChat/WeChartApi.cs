    ...
    public class WeChartApi
    {
        ...
        public static async Task<string> GetWXACode(WXACodeParam param, string token)
        {
            ...
            catch (Exception ex)
            {
                ...
                //取得当前方法名    
                string methedName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                Logger.Error($"获取微信小程序码失败 {sendUrl}：{ex.Message} {Environment.NewLine} at {className}.{methedName}:{Environment.NewLine} {p}");
            }
            return result;
        }
        ...
        public static async Task<WechatJsAPITicketResponse> GetJsAPITicket(string appId, string secret)
        {
            ...
            if (!has)
            {
                ...
                if (response.Errcode != 0)
                {
                    ...
                    //取得当前方法名    
                    string methedName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                    Logger.Error($"获取微信JS接口的临时票据出错 {url}：{response.Errmsg} {Environment.NewLine} at {className}.{methedName}:");
                }
                else
                {
                    await RedisHelper.Database.StringSetAsync(key, JsonConvert.SerializeObject(response), new TimeSpan((response.ExpiresIn - 60 * 5) * (long)10000000));
        ...
        public static async Task<WeChatResponseDto> SendCustomMsg(CustomSendMsgBase pram, string token)
        {
            ...
            if (response.Errcode != 0)
            {
                ...
                //取得当前方法名    
                string methedName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                Logger.Error($"发送客服消息报错 {sendUrl}：{response.Errmsg} {Environment.NewLine} at {className}.{methedName}:{Environment.NewLine} {p}");
            }
            return response;
        }
        ...
        /// 获取公众号二维码
        /// </summary>
        public static async Task<CreateQRCodeResponseDto> CreateQRCodeAsync(CreateQRCodeRequestDto pram, string token)
        {
            ...
            if (response.Errcode != 0)
            {
                ...
                //取得当前方法名    
                string methedName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                Logger.Error($"获取公众号二维码报错 {sendUrl}：{response.Errmsg} {Environment.NewLine} at {className}.{methedName}:{Environment.NewLine} {p}");
            }
            return response;
        }
    }
